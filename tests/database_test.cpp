...
TEST_F(DatabaseTest, BackupPublishesASelfContainedVerifiedDatabaseWithoutAWal) {
  ...
  ASSERT_TRUE(database.Put("overflow", large).Ok());

  ASSERT_TRUE(database.CreateBackup(second_db_path_).Ok());
  ASSERT_TRUE(std::filesystem::exists(second_db_path_));
  EXPECT_FALSE(std::filesystem::exists(tinydb::Wal::PathFor(second_db_path_)));

  // The source remains open and independently writable. The backup opens from
  // its database file alone and retains exactly the checkpointed state.
  ASSERT_TRUE(database.Put("later", "source-only").Ok());
  auto backup = tinydb::Database::Open(second_db_path_).value();
  EXPECT_EQ(backup.Get("small").value(), "value");
  EXPECT_EQ(backup.Get("overflow").value(), large);
  EXPECT_EQ(backup.Get("later").value(), std::nullopt);
}

TEST_F(DatabaseTest, BackupNeverReplacesAnExistingDestination) {
  auto database = tinydb::Database::Open(db_path_).value();
  ASSERT_TRUE(database.Put("key", "value").Ok());
  {
    auto existing = std::ofstream(second_db_path_, std::ios::binary);
    existing << "application-owned";
  }

  const auto status = database.CreateBackup(second_db_path_);
  EXPECT_EQ(status.Code(), tinydb::StatusCode::InvalidArgument);
  auto contents = std::ifstream(second_db_path_, std::ios::binary);
  EXPECT_EQ(std::string(std::istreambuf_iterator<char>{contents}, {}), "application-owned");
}

TEST_F(DatabaseTest, BackupKeepsReadersLiveWhileADurableWriterWaitsToPublish) {
  auto database = tinydb::Database::Open(db_path_).value();
  ASSERT_TRUE(database.Put("key", "value").Ok());
  ...
  auto copy_started = false;
  auto release_copy = false;
  auto writer_durable = false;
  auto backup_status = tinydb::Status{};
  {
    auto hook = ScopedSyscallHook{[&](const tinydb::io::Call &call) -> std::optional<tinydb::io::Fault> {
      if (call.syscall == tinydb::io::Syscall::Pwrite && call.path != db_path_ &&
          call.path.string().starts_with(second_db_path_.string())) {
        auto lock = std::unique_lock(mutex);
        copy_started = true;
        changed.notify_all();
        changed.wait(lock, [&] { return release_copy; });
      }
      if (call.syscall == tinydb::io::Syscall::Fsync && call.path == tinydb::Wal::PathFor(db_path_)) {
    ...
    auto backup = std::thread([&] { backup_status = database.CreateBackup(second_db_path_); });
    {
      auto lock = std::unique_lock(mutex);
      changed.wait(lock, [&] { return copy_started; });
    }

    // Backup holds only the publication boundary. The old snapshot remains
    // usable while a writer prepares and synchronizes its WAL transaction.
    EXPECT_EQ(reader->Get("key").value(), "value");
    auto writer = database.BeginWrite().value();
    ASSERT_TRUE(writer.Put("concurrent", "published-later").Ok());
    auto commit_result = tinydb::Result<tinydb::CommitInfo>{std::unexpected(tinydb::Status::Busy("not started"))};
    auto commit = std::thread([&] { commit_result = std::move(writer).Commit(); });
    {
      auto lock = std::unique_lock(mutex);
      changed.wait(lock, [&] { return writer_durable; });
    }
    EXPECT_EQ(reader->Get("concurrent").value(), std::nullopt);
    reader.reset();
    ...
    backup.join();
    commit.join();
    ASSERT_TRUE(commit_result.has_value()) << commit_result.error().ToString();
  }
  ...
  EXPECT_EQ(backup.Get("concurrent").value(), std::nullopt);
}

TEST_F(DatabaseTest, FailedBackupCopyRemovesItsPrivateStagingFile) {
  auto database = tinydb::Database::Open(db_path_).value();
  ASSERT_TRUE(database.Put("key", "value").Ok());
  {
    auto hook = ScopedSyscallHook{[&](const tinydb::io::Call &call) -> std::optional<tinydb::io::Fault> {
  ...
  EXPECT_FALSE(std::filesystem::exists(second_db_path_));
  auto parent = second_db_path_.parent_path();
  for (const auto &entry : std::filesystem::directory_iterator(parent)) {
    EXPECT_FALSE(entry.path().filename().string().starts_with(second_db_path_.filename().string() + ".tinydb-"));
  }
}

TEST_F(DatabaseTest, BackupValidationRejectsDamagedSourceBytesBeforePublication) {
  auto database = tinydb::Database::Open(db_path_).value();
  ASSERT_TRUE(database.Put("key", "value").Ok());
  ASSERT_TRUE(database.Checkpoint().Ok());
  {
    auto file = std::fstream(db_path_, std::ios::in | std::ios::out | std::ios::binary);
    file.seekg(static_cast<std::streamoff>(tinydb::FIRST_DATA_PAGE_ID * tinydb::PAGE_SIZE));
    char byte = 0;
    file.get(byte);
