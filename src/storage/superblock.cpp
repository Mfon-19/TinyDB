#include "storage/superblock.h"

#include "storage/encoding.h"
#include "util/crc32.h"

#include <algorithm>
#include <array>
#include <cstddef>
#include <cstdint>
#include <optional>
#include <span>
#include <string>

namespace tinydb::storage {
namespace {

static_assert(superblock_offset::ENCODED_BYTES <= PAGE_SIZE);

/*
** Return a description of the first invalid field or relationship in
** superblock, or nullopt if the metadata is valid.  The encoder reports the
** description as InvalidArgument, whereas the decoder reports the same
** condition as Corruption.
*/
auto InvalidSuperblock(const Superblock &superblock) -> std::optional<std::string> {
  if (superblock.database_uuid == DatabaseUuid{}) {
    return "database UUID is zero";
  }
  if (superblock.generation == 0) {
    return "generation is zero";
  }
  if (superblock.logical_page_count < FIRST_FORMAT_DATA_PAGE_ID) {
    return "logical page count omits one or both superblocks";
  }

  const auto valid_reference = [&](page_id_t page_id) {
    return page_id == 0 || (page_id >= FIRST_FORMAT_DATA_PAGE_ID && page_id < superblock.logical_page_count);
  };
  if (!valid_reference(superblock.root_page_id)) {
    return "root page ID is outside the logical page range";
  }
  if (!valid_reference(superblock.allocator_root_page_id)) {
    return "allocator root page ID is outside the logical page range";
  }
  return std::nullopt;
}

}  // namespace
auto EncodeSuperblock(const Superblock &superblock) -> Result<SuperblockPage> {
  if ((superblock.required_features & ~SUPPORTED_REQUIRED_FEATURES) != 0) {
    return std::unexpected(Status::UnsupportedFormat("cannot encode unsupported required superblock features"));
  }
  if (const auto invalid = InvalidSuperblock(superblock)) {
    return std::unexpected(Status::InvalidArgument(*invalid));
  }

  auto page = SuperblockPage{};
  auto bytes = std::span<std::byte>{page};
  PutBytesUnchecked(bytes, superblock_offset::MAGIC, SUPERBLOCK_MAGIC);
  PutLittleEndianUnchecked(bytes, superblock_offset::FORMAT_MAJOR, FORMAT_MAJOR);
  PutLittleEndianUnchecked(bytes, superblock_offset::FORMAT_MINOR, FORMAT_MINOR);
  PutLittleEndianUnchecked(bytes, superblock_offset::PAGE_SIZE, static_cast<std::uint32_t>(PAGE_SIZE));
  PutLittleEndianUnchecked(bytes, superblock_offset::REQUIRED_FEATURES, superblock.required_features);
  PutLittleEndianUnchecked(bytes, superblock_offset::OPTIONAL_FEATURES, superblock.optional_features);
  PutBytesUnchecked(bytes, superblock_offset::DATABASE_UUID, superblock.database_uuid);
  PutLittleEndianUnchecked(bytes, superblock_offset::GENERATION, superblock.generation);
  PutLittleEndianUnchecked(bytes, superblock_offset::CHECKPOINT_LSN, superblock.checkpoint_lsn);
  PutLittleEndianUnchecked(bytes, superblock_offset::ROOT_PAGE_ID, superblock.root_page_id);
  PutLittleEndianUnchecked(bytes, superblock_offset::ALLOCATOR_ROOT_PAGE_ID, superblock.allocator_root_page_id);
  PutLittleEndianUnchecked(bytes, superblock_offset::LOGICAL_PAGE_COUNT, superblock.logical_page_count);
  PutLittleEndianUnchecked(bytes, superblock_offset::CHECKSUM, Crc32WithZeroedU32(bytes, superblock_offset::CHECKSUM));
  return page;
}

auto DecodeSuperblock(std::span<const std::byte> page) -> Result<Superblock> {
  if (page.size() != PAGE_SIZE) {
    return std::unexpected(Status::Corruption("superblock is not exactly one page"));
  }
  if (!std::ranges::equal(SUPERBLOCK_MAGIC, page.subspan(superblock_offset::MAGIC, SUPERBLOCK_MAGIC.size()))) {
    return std::unexpected(Status::UnsupportedFormat("unrecognized TinyDB superblock magic"));
  }

  const auto stored_checksum = GetLittleEndianUnchecked<std::uint32_t>(page, superblock_offset::CHECKSUM);
  if (stored_checksum != Crc32WithZeroedU32(page, superblock_offset::CHECKSUM)) {
    return std::unexpected(Status::Corruption("superblock checksum mismatch"));
  }

  const auto major = GetLittleEndianUnchecked<std::uint16_t>(page, superblock_offset::FORMAT_MAJOR);
  const auto minor = GetLittleEndianUnchecked<std::uint16_t>(page, superblock_offset::FORMAT_MINOR);
  const auto page_size = GetLittleEndianUnchecked<std::uint32_t>(page, superblock_offset::PAGE_SIZE);
  const auto required = GetLittleEndianUnchecked<std::uint64_t>(page, superblock_offset::REQUIRED_FEATURES);
  const auto optional = GetLittleEndianUnchecked<std::uint64_t>(page, superblock_offset::OPTIONAL_FEATURES);
  if (major != FORMAT_MAJOR || minor > FORMAT_MINOR || page_size != PAGE_SIZE ||
      (required & ~SUPPORTED_REQUIRED_FEATURES) != 0) {
    return std::unexpected(Status::UnsupportedFormat("unsupported TinyDB superblock version or features"));
  }

  auto superblock = Superblock{};
  GetBytesUnchecked(page, superblock_offset::DATABASE_UUID, superblock.database_uuid);
  const auto generation = GetLittleEndianUnchecked<std::uint64_t>(page, superblock_offset::GENERATION);
  const auto checkpoint_lsn = GetLittleEndianUnchecked<std::uint64_t>(page, superblock_offset::CHECKPOINT_LSN);
  const auto reserved = GetLittleEndianUnchecked<std::uint64_t>(page, superblock_offset::RESERVED);
  const auto root_page_id = GetLittleEndianUnchecked<page_id_t>(page, superblock_offset::ROOT_PAGE_ID);
  const auto allocator_root_page_id =
      GetLittleEndianUnchecked<page_id_t>(page, superblock_offset::ALLOCATOR_ROOT_PAGE_ID);
  const auto logical_page_count = GetLittleEndianUnchecked<page_id_t>(page, superblock_offset::LOGICAL_PAGE_COUNT);
  if (reserved != 0) {
    return std::unexpected(Status::Corruption("nonzero reserved superblock field"));
  }
  superblock.generation = generation;
  superblock.checkpoint_lsn = checkpoint_lsn;
  superblock.root_page_id = root_page_id;
  superblock.allocator_root_page_id = allocator_root_page_id;
  superblock.logical_page_count = logical_page_count;
  superblock.required_features = required;
  superblock.optional_features = optional;

  if (const auto nonzero = std::ranges::find_if(page.subspan(superblock_offset::ENCODED_BYTES),
                                                [](std::byte byte) { return byte != std::byte{0}; });
      nonzero != page.end()) {
    return std::unexpected(Status::Corruption("nonzero reserved superblock bytes"));
  }
  if (const auto invalid = InvalidSuperblock(superblock)) {
    return std::unexpected(Status::Corruption(*invalid));
  }
  return superblock;
}

auto SelectSuperblock(std::span<const std::byte> page_a,
                      std::span<const std::byte> page_b) -> Result<SelectedSuperblock> {
  auto first = DecodeSuperblock(page_a);
  auto second = DecodeSuperblock(page_b);

  if (first && second) {
    // Equal generations provide no ordering information. Different metadata
    // would leave no safe way to decide which slot was published.
    if (first->generation == second->generation && *first != *second) {
      return std::unexpected(Status::Corruption("superblocks have the same generation but different state"));
    }
    if (second->generation > first->generation) {
      return SelectedSuperblock{.value = *second, .slot = SuperblockSlot::B};
    }
    return SelectedSuperblock{.value = *first, .slot = SuperblockSlot::A};
  }

  if (first) {
    return SelectedSuperblock{.value = *first, .slot = SuperblockSlot::A};
  }
  if (second) {
    return SelectedSuperblock{.value = *second, .slot = SuperblockSlot::B};
  }

  if (first.error().Code() == StatusCode::UnsupportedFormat && second.error().Code() == StatusCode::UnsupportedFormat) {
    return std::unexpected(Status::UnsupportedFormat("neither superblock is valid"));
  }
  return std::unexpected(Status::Corruption("neither superblock is valid"));
}

}  // namespace tinydb::storage
