...
constexpr auto CRC32_TABLE = [] {
  ...
  for (std::uint32_t value = 0; value < table[0].size(); ++value) {
    ...
    for (unsigned bit = 0; bit < 8; ++bit) {
      remainder =
          (remainder >> 1U) ^ ((remainder & 1U) != 0 ? CRC32_POLYNOMIAL : 0U);
    }
  ...
  for (std::size_t slice = 1; slice < table.size(); ++slice) {
    for (std::size_t value = 0; value < table[slice].size(); ++value) {
      ...
      table[slice][value] = (remainder >> 8U) ^ table[0][remainder & 0xFFU];
    }
  }
  return table;
}();
...
void Crc32Accumulator::Update(std::span<const char> bytes) noexcept {
  while (bytes.size() >= 16) {
    ...
    remainder_ =
        CRC32_TABLE[15][first & 0xFFU] ^
        CRC32_TABLE[14][(first >> 8U) & 0xFFU] ^
        CRC32_TABLE[13][(first >> 16U) & 0xFFU] ^
        CRC32_TABLE[12][first >> 24U] ^ CRC32_TABLE[11][second & 0xFFU] ^
        CRC32_TABLE[10][(second >> 8U) & 0xFFU] ^
        CRC32_TABLE[9][(second >> 16U) & 0xFFU] ^
        CRC32_TABLE[8][second >> 24U] ^ CRC32_TABLE[7][third & 0xFFU] ^
        CRC32_TABLE[6][(third >> 8U) & 0xFFU] ^
        CRC32_TABLE[5][(third >> 16U) & 0xFFU] ^ CRC32_TABLE[4][third >> 24U] ^
        CRC32_TABLE[3][fourth & 0xFFU] ^
        CRC32_TABLE[2][(fourth >> 8U) & 0xFFU] ^
        CRC32_TABLE[1][(fourth >> 16U) & 0xFFU] ^ CRC32_TABLE[0][fourth >> 24U];
    bytes = bytes.subspan(16);
  }
  for (const char byte : bytes) {
    remainder_ =
        (remainder_ >> 8U) ^
        CRC32_TABLE[0][(remainder_ ^ static_cast<unsigned char>(byte)) & 0xFFU];
  }
}
