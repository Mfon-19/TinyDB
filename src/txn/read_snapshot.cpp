...
auto SnapshotCursor::First() -> Status {
  auto cursor = BTreeCursor::First(pages_, root_page_id_, high_water_page_id_);
  if (!cursor) {
    return cursor.error();
  }
...
auto SnapshotCursor::Seek(std::string_view key) -> Status {
  auto cursor = BTreeCursor::Seek(pages_, root_page_id_, high_water_page_id_, key);
  if (!cursor) {
    return cursor.error();
  }
  cursor_ = std::move(*cursor);
...
auto ReadSnapshot::First() -> Result<SnapshotCursor> {
  auto cursor = BTreeCursor::First(pages_, State().root_page_id, State().high_water_page_id);
  if (!cursor) {
    return std::unexpected(std::move(cursor).error());
  }
  return SnapshotCursor(snapshot_, pages_, State().root_page_id, State().high_water_page_id, std::move(*cursor));
}

auto ReadSnapshot::Seek(std::string_view key) -> Result<SnapshotCursor> {
  auto cursor = BTreeCursor::Seek(pages_, State().root_page_id, State().high_water_page_id, key);
  if (!cursor) {
    return std::unexpected(std::move(cursor).error());
  }
